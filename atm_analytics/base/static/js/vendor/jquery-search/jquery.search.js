    ...
    $.extend($.searcher, {
        ...
        prototype : {
            ...
            getConcurrencesNumber : function(){
                ...
                var concurrences = this._filter().length;
                return  concurrences;
            },

            findConcurrence : function(searchOrder){
                var beforefn = this.settings.beforeSearch.apply(this, $.makeArray(this.settings.text) );
                ...
                if (concurrencesNumber != 1){
                    ...
                    if (_searchOrder === true || _searchOrder === "up"){
                        ...
                        //this._debug(position+" "+concurrencesNumber);
                    }
                    //otherwise
                    else {
                ...
                var textEl = this._filter().eq(position);
                this._highlightSelected(position);
                this.currentPosition = position;

                //scroll to element in case of scrollTo jQuery plugin presence
                if (this.settings.scrollTo && $.fn.scrollTo){
                    $('#'+this.settings.target+' p').scrollTo($("#m"+position),300);
                }
            ...
            _filter : function(){
                ...
                var result = $findResult.contents("mark");
                return result;
            },

            _highlightSelected : function(position){
                var textHolder = $('#'+this.settings.target).find('p').html();
                var textToSelect = this.settings.text;

                var startPosition = $('#'+this.settings.target).find('p').html().indexOf("m"+position);
                var endPosition = startPosition + textToSelect.length;

                $("#m"+position).replaceWith(function() { return "<span class='highlight' id='m"+position+"'>"+$(this).text()+"</span>"; });
                if(position>1){
                    $("#m"+position-1).replaceWith(function() { return "<mark id='m"+(position-1)+"'>"+$(this).contents()+"</mark>"; })
                }
            },
